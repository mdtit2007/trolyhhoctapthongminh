...
namespace ungdunghocthongminh.Areas.Admin.Controllers
{
    public class AdviseController : Controller
    {
        ...
        public ActionResult Index(string Searchtext, int? page)
        {
            ...
            IEnumerable<Advise> items = db.Advise.OrderByDescending(x => x.Id);
        ...
        public ActionResult Add(Advise model)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit(Advise model)
        {
            if (ModelState.IsValid)
            {

                model.Alias = ungdunghocthongminh.Models.Common.Filter.FilterChar(model.Title);
                db.Advise.Attach(model);
        ...
        public ActionResult DeleteAll(string ids)
        {
            if (!string.IsNullOrEmpty(ids))
            {
                ...
                if (items != null && items.Any())
                {
                    ...
                    {
                        var obj = db.Advise.Find(Convert.ToInt32(item));
                        db.Advise.Remove(obj);
                        db.SaveChanges();
                    }
                }
                return Json(new { success = true });
